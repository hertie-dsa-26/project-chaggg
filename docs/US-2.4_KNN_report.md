...
| 3 | CI’da coverage (`coverage run` + `coverage report`, `fail_under`) | **Tamam** (12 test; rapor ~%80 satır, push yok) |
...
- Dev dependency **coverage**; `[tool.coverage.run]` / `[tool.coverage.report]` (`source=src`, `fail_under=78`).
